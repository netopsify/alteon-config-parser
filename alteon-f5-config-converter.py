...
with open(runtime_vars_dir + "/real_servers_parsed_results.json", "w+",) as f:
    f.write(json.dumps(real_servers[0][0]))
...
with open(runtime_vars_dir + "/virtual_servers_parsed_results.json", "w+",) as f:
    f.write(json.dumps(virtual_servers[0][0]))
...
with open(runtime_vars_dir + "/ssl_cert_bundle_parsed_results.json", "w+",) as f:
    f.write(json.dumps(ssl_cert_bundle[0][0]))
...
with open(runtime_vars_dir + "/ssl_certs_parsed_results.json", "w+",) as f:
    f.write(json.dumps(ssl_certs[0][0]))
...
with open(runtime_vars_dir + "/ssl_policies_parsed_results.json", "w+",) as f:
    f.write(json.dumps(ssl_policies[0][0]))
...
# Declaring a new object vips for later use in the program
# vips = vips
...
with open(runtime_vars_dir + "/nodes_seq_dict.json", "w+",) as f:
    f.write(json.dumps(nodes_seq_dict))

with open(runtime_vars_dir + "/nodes_name_dict.json", "w+",) as f:
    f.write(json.dumps(nodes_name_dict))
...
with open(runtime_vars_dir + "/pools_seq_dict.json", "w+",) as f:
    f.write(json.dumps(pools_seq_dict))

with open(runtime_vars_dir + "/pools_name_dict.json", "w+",) as f:
    f.write(json.dumps(pools_name_dict))

vips_vip_ip_dict = {}
vips_vip_seq_dict = {}
with open(runtime_vars_dir + "/vips_vip_ip_dict.json", "w+",) as f:
    f.write(json.dumps(virtual_servers[0][0]['virt']))
...
with open(runtime_vars_dir + "/vips_vip_seq_dict.json", "w+",) as f:
    f.write(json.dumps(vips_vip_seq_dict))
...
with open(runtime_vars_dir + "/pbind_cookie_vip_seq_dict.json", "w+",) as f:
    f.write(json.dumps(pbind_cookie_vip_seq_dict))

with open(runtime_vars_dir + "/pbind_cookie_vip_service_dict.json", "w+",) as f:
    f.write(json.dumps(pbind_cookie_vip_service_dict))
...
with open(runtime_vars_dir + "/pbind_sslid_vip_seq_dict.json", "w+",) as f:
    f.write(json.dumps(pbind_sslid_vip_seq_dict))
...
if alteon_state_dump:
    ...
    with open(runtime_vars_dir + "/real_servers_state_dump_parser_results.json", "w+",) as f:
        f.write(json.dumps(real_servers_state_dump[0][0]))
    ...
    with open(runtime_vars_dir + "/virtual_servers_state_dump_parser_results.json", "w+",) as f:
        f.write(json.dumps(virtual_servers_state_dump[0][0]))
    ...
    for vip, vip_config in vips.items():
        if vip_config['config_state'] == 'ena':
            for vip_port, srv_config in vip_config['services'].items():
                ...
                _ = vip_dict.update({'virt_seq': srv_config["virt_seq"]})
                _ = vip_dict.update({'vip_ip': vip})
                _ = vip_dict.update({'vip_port': vip_port})
                if vip_config['vip_name'] != 'None':
                    vip_name = regex_replace(
                        vip_config['vip_name'], "[^A-Za-z0-9-_.]", "")
                    _ = vip_dict.update(
                        {'vip_name': "vs-tcp-" + str(vip_port) + "-" + vip_name + "-" + str(vip)})
                    _ = vip_dict['pool'].update(
                        {'name': "pool-tcp-" + str(vip_port) + "-" + vip_name + "-" + str(vip)})
                ...
                for node in nodes_list:
                    ...
                    _ = vip_dict['pool']['members'].append({
                ...
                if 'pbind' in srv_config.keys() and 'ssl_profile' not in srv_config.keys():
                    if srv_config['pbind'] != "None":
                        if srv_config['pbind'] == "sslid":
                            _ = vip_dict.update({'vip_type': "b"})
                        elif srv_config['pbind'] == "clientip":
                            _ = vip_dict.update({'vip_type': "e"})
                        else:
                            _ = vip_dict.update({'vip_type': "a"})
                    else:
                        _ = vip_dict.update({'vip_type': "a"})
                elif 'pbind' in srv_config.keys() and 'ssl_profile' in srv_config.keys():
                    if srv_config['pbind'] != "None":
                        if srv_config['pbind'] == "cookie":
                            _ = vip_dict.update({'vip_type': "c"})
                            _ = vip_dict.update(
                                {'ssl_profile': srv_config['ssl_profile']})
                            _ = vip_dict['ssl_profile'].update({
                    ...
                    _ = vip_dict.update({'vip_type': "d"})

                else:
                    _ = vip_dict.update({'vip_type': "a"})

                _ = all_vips.append(vip_dict)
    return all_vips
